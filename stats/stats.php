...
class picoStats {
	...
	function trackPage($account,$page,$extraParams="") {
		...
		// delete any entries that are into the exclusion table
		$query=$db->ExecuteSQL("delete from analytics where exists(select * from iptable where iptable.ipaddress=analytics.remote_addr)");
	...
	function excludeIP($ipaddress) {
		$db=new MySQL($this->database,$this->username,$this->password);
		$result=$db->Insert(array("ipaddress"=>$_SERVER['REMOTE_ADDR']),"iptable",true);
		if(!$result) {
			trigger_error("Db error in Pico stats:".$db->lastError." query:".$db->lastQuery,E_USER_ERROR);
		}
		$db->CloseConnection();
