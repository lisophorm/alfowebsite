...
class MySQL {
	...
	function InsertUpdate($vars,$table,$index) {
		$resulto=$this->Select($table,$index);
		if($this->records>0) {
			//echo "record exists<br/>";
			return $this->Update($table,$vars,$index);
		} else {
			//echo "new record<br/>";
			return $this->Insert($vars,$table);
		}
		
	}
	
	// Adds a record to the database based on the array key names
	function Insert($vars, $table, $ignore=false,$exclude = ''){
		...
		$query = "INSERT ".$ignorequery." INTO `{$table}` SET ";
	...
	function Update($table, $set, $where=NULL, $exclude = ''){
		// Catch Exceptions
		if(trim($table) == '' || !is_array($set)){
			return false;
		}
		if($exclude == ''){
			$exclude = array();
...
function log_error($error,$error_description="",$extrainfo="",$item_id=0) {
	$database=new MySQL(DB,DBUSER,DBPASS);
	$resulto=$database->Insert(array(),"boxerrorlog");
	if(!$resulto) {
		echo "error:".$database->lastError;
	}
	sendmail("Gino Tronico","gino@wmgllc.com","Alfonso Florio","aflorio@wmgllc.com","BOX error - ".$error,$error."<br/>".$error_description."<br/>".$extrainfo."<br/>".$item_id);
